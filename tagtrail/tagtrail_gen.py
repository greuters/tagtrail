...
class Model():
    ...
    def initializeSheets(self):
        ...
        Each sheet is assigned to a category to be able to show the user what
...
class GUI(gui_components.BaseGUI):
    ...
    def __addCategoryFrame(self, parent, title, previousState, newState, tooltip):
        """
        ...
        tkinter.Label(frame, text=title).pack()
        rowIdx = 0
        for sheet in self.model.sheets:
            if sheet.previousState != previousState or sheet.newState != newState:
                continue
            rowIdx += 1
            tkinter.Label(frame, text=sheet.filename).pack(padx = self.padx, pady = self.pady)
